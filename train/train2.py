...
def train(data_dir, model_dir, args):
    ...
    # -- data_loader
    datasets = dataset.kfold_split(5, args.dataset.oversampling, args.dataset.oversample_category, args.dataset.oversample_weight)
    
    for i, (train_set, val_set) in enumerate(datasets):
        print(f"{i+1}번 Fold 학습 중...")
        ...
        # -- logging
        fold_save_dir = os.path.join(save_dir, f"fold{i+1}")
        os.makedirs(fold_save_dir, exist_ok=True)
        with open(os.path.join(fold_save_dir, 'config.json'), 'w', encoding='utf-8') as f:
        ...
        for epoch in range(args.train.epochs):
            ...
            with torch.no_grad():
                ...
                if val_f1score > best_val_f1score:
                    print(f"New best model for val f1score : {val_f1score:4.2}! saving the best model..")
                    torch.save(model, f"{fold_save_dir}/best.pth")
                    best_val_acc = val_acc
                    best_val_f1score = val_f1score
                    best_val_preds = pred
                    best_val_targets = target
                    
                torch.save(model, f"{fold_save_dir}/last.pth")
                
                print(
                    f"[Val] acc : {val_acc:4.2%}, loss: {val_loss:4.2}, f1score: {val_f1score:4.2} || "
                    f"best acc : {best_val_acc:4.2%}, best loss: {best_val_loss:4.2}, best f1score: {best_val_f1score:4.2} \n"
                )
                if args.wandb.logging:
        ...
        if args.wandb.logging:
            ...
            wandb.run.summary["best_val_f1score"] = best_val_f1score
            wandb.finish()
